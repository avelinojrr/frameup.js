...
export async function generateScaffolding(config) {
    ...
    if (!mappedArchitecture || !mappedLanguage || !mappedDb) {
        throw new Error(`Invalid configuration: Architecture: ${architecture}, Language: ${language}, Database: ${database}`);
    }
    ...
    const dbDestinationPath = path.join(process.cwd(), projectName, 'src', 'database/', 'db.js');  // Copy the selected db config to src/

    if (fs.existsSync(dbTemplatePath)) {
        fs.copyFileSync(dbTemplatePath, dbDestinationPath);
        console.log(`Database configuration for ${database} has been added to src/db.js.`);
    } else {
