...
export async function getStackConfig() {
    ...
    const {
        ...
        async format(input) {
            return input.trim() ? frameupColors.outputColor(input) : frameupColors.outputColor('frameup-project');
        }
    })
    ...
    const {
        promptAddArchitecture
    } = await prompt({
        type: 'confirm',
        name: 'promptAddArchitecture',
        message: frameupColors.inputColor('Would you like to add an architecture?'),
        initial: true,
        format(input) {
    ...
    const {
        promptAddDesignPattern
    } = await prompt({
        type: 'confirm',
        name: 'promptAddDesignPattern',
        message: frameupColors.inputColor('Would you like to add a design pattern?'),
        initial: true, 
        format(input) {
            return input ? frameupColors.outputColor('Yes') : frameupColors.outputColor('No');
        }
    ...
    if (confirmDatabase) {
        ...
        const databaseType = await promptUser({
            name: 'databaseType',
            message: frameupColors.inputColor('Select a database type (SQL or NoSQL):'),
        ...
        const selectedDatabase = await promptUser({
            name: 'database',
            message: frameupColors.inputColor(`Select a ${databaseType} database:`),
    ...
    const confirmTools = await prompt({
        type: 'confirm',
        name: 'addTools',
        message: frameupColors.inputColor('Would you like to add some support tools?'),
        initial: true,
        format(input) {
            return input ? frameupColors.outputColor('Yes') : frameupColors.outputColor('No');
        }
    ...
    if (confirmTools.addTools) {
        const selectedTools = await prompt({
            type: 'multiselect',
            name: 'tools',
            message: frameupColors.inputColor('Select the support tools you want to include:'),
            ...
            validate(value) {
                return value.length > 0 ? true : 'Select at least one tool or press ESC to skip.';
            },
            result(names) {
                return this.choices.filter(choice => names.includes(choice.name)).map(choice => choice.value);
            },
            format(input) {
                const selected = toolsChoices.filter(tool => input.includes(tool.name));
